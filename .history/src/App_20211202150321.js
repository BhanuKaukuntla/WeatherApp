...
function App() {
  ...
  return (
    <div>
      <section class="relative bg-white-900  min-h-screen">

        <div class="relative container pt-12 px-4 mb-20 mx-auto text-center">
          
          <h2 class="mt-8 mb-8 lg:mb-12 text-black text-4xl lg:text-6xl font-semibold">
            Weather App
          </h2>
          ...
          <input
            ...
            onChange={e => setCity(e.target.value)}
            placeholder="Search City"
            class="relative z-10 inline-block w-full md:w-auto mb-2  px-3 py-2 mr-4  font-medium leading-normal bg-transparent border-2 rounded-lg text-black-400 "
          ></input>
          <button
            onClick={() => dispatch(fetchWeatherAction(city))}
            type="button"
            className="inline-flex items-center px-3 pr-3 28 text-center py-3 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-black-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            Search
          </button>
        </div>
              ...
              <div class="w-full md:w-1/3 px-4">
                <div class="p-8 border border-blue-800 rounded-lg">
                  ...
                  <h1 class="text-black-300 text-center text-4xl mb-10">
                    {Math.ceil(Number(weather?.main.temp))}{" "}
                    <span class="mb-6 text-xl text-black font-semibold">°C</span>
                  </h1>
                  <h3 class="mb-6 text-xl text-black font-semibold">
                    {weather?.name}, {weather?.sys?.country}
                  </h3>
                  <p class="mb-8 text-black-300">
                    The weather condition in {weather?.name},{" "}
                    {weather?.sys?.country} is described as :{" "}
                    {weather?.weather[0].description} with a temperature of{" "}
                    {Math.ceil(Number(weather?.main.temp))} °C and a humidity of{" "}
